...
cl.login(token="Masukin_Token_Lu")
cl.loginResult()
...
Bots=[midCL]
owner=["u6714a051857a68c74548642e48c563b6","ued156c86ffa56024c0acba16f7889e6d"]
admin=["u6714a051857a68c74548642e48c563b6","ued156c86ffa56024c0acba16f7889e6d"]
staff=["ued156c86ffa56024c0acba16f7889e6d"]
            ...
            if msg.contentType == 0:
                if msg.text == None:
                  ...
                  if msg.from_ in admin:
                    ...
                    cl.sendText(msg.to, "Jumlah Members :"+str(jml))
                  else:
                    cl.sendText(msg.to, "Perintah Ini Cuma Buat Admin & Owner Njiir\n😂😂😂")
                  ...
                  if msg.from_ in owner:
                    ...
                    if cmd == "on":
                      if mimic["status"] == False:
                        mimic["status"] = True
                        k1.sendText(msg.to,"Mimic On Boss")
                      else:
                        k1.sendText(msg.to,"Mimic Already On Boss")
                    elif cmd == "off":
                      if mimic["status"] == True:
                        mimic["status"] = False
                        k1.sendText(msg.to,"Mimic Off Boss")
                      else:
                        k1.sendText(msg.to,"Mimic Already off")
                    elif cmd == "list":
                      if mimic["target"] == {}:
                        k1.sendText(msg.to,"No target")
                      else:
                        mc = "Target Mimic\n"
                        mids = []
                        for s in range(len(mimic["target"])):
                            mids.append(mimic["target"][s])
                        cmids = k1.getContacts(mids)
                        for x in range(len(cmids)):
                            mc += "\n["+str(x)+"]"+cmids[x].displayName
                        k1.sendText(msg.to,mc)
                    elif "add " in cmd:
                      ...
                      if targets == []:
                        k1.sendText(msg.to,"No targets")
                      else:
                        for target in targets:
                          try:
                            mimic["target"][target] = True
                            k1.sendText(msg.to,"Success added target")
                            k1.sendMessageWithMention(msg.to,target)
                            break
                          except:
                            k1.sendText(msg.to,"Ready To Following Chat From Target Boss")
                            break
                    elif "del " in cmd:
                      ...
                      if targets == []:
                        ...
                        for target in targets:
                          try:
                            ...
                            k1.sendText(msg.to,"Success deleted target")
                            k1.sendMessageWithMention(msg.to,target)
                            break
                          except:
                            k1.sendText(msg.to,"Im Not Following Again Boss\nTired, Not Get Money")
                            break
