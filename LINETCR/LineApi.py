...
class LINE:
  ...
  def __init__(self):
    ...
    self._headers = {'X-Line-Application': 'CHROMEOS 8.2.2 KOPLAXS x64'}    
  ...
  def login(self, mail=None, passwd=None, cert=None, token=None, qr=False, callback=None):
    ...
    self._headers = {
              'X-Line-Application': 'CHROMEOS 8.2.2 KOPLAXS x64', 
  ...
  def mention(self, to, nama):
        ...
        akh = int(0)
        nm = nama
        myid = self.Talk.client.getProfile().mid
        if myid in nm:    
            nm.remove(myid)
        for mm in nm:
          akh = akh + 6
        ...
        try:
            ...
            msg.contentType = 0
            self.Talk.client.sendMessage(0, msg)
  ...
  def loginResult(self, callback=True):
    if callback is True:
      ...
      print("===============[ONE PIECE TEAM]================")
      print("        Thanks for LINETCR TEAM")
      print("===============[© By_Hanavy_Koplaxs]================")
      print("mid -> " + prof.mid)
